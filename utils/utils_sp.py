...
def text2idx_sentpiece(sp_model):
    def func(text):
        # return sp_model.encode(text[:125], add_bos=True, add_eos=True)
        # return [SOS_IDX] + sp_model.encode(text)[:125] + [EOS_IDX]
...
def idx2text_field(field):
    pad_sos_bos = [1, 2, 3] # specific for vocabs
    def func(idxs):
        return " ".join([field.vocab.itos[idx] for idx in idxs
                         if idx not in pad_sos_bos])
    return func
...
def translate(model, src_text, src_text2idx, trg_idx2text):
    ...
    src_idx = torch.tensor(src_text2idx(src_text))
